...
PANEL_REPOSITORY=${PANEL_REPOSITORY:-wemwall2009/sinhysteria2-new}
PANEL_BRANCH=${PANEL_BRANCH:-main}
PANEL_LETSENCRYPT=${PANEL_LETSENCRYPT:-true}
...
if [[ -n ${PANEL_CERT_FILE:-} && -n ${PANEL_KEY_FILE:-} ]]; then
  ...
  if [[ ${PANEL_LETSENCRYPT} == "true" ]]; then
    if ! command -v socat >/dev/null 2>&1; then
      echo "未找到 socat，无法申请 Let’s Encrypt 证书，回退到自签名证书。"
    elif ss -ltn | awk '$4 ~ /:80$/ {found=1} END {exit found ? 0 : 1}'; then
      echo "TCP 80 已被占用，无法申请 Let’s Encrypt 证书，回退到自签名证书。"
    else
      if [[ ! -x /root/.acme.sh/acme.sh ]]; then
        if ! curl -fsSL https://get.acme.sh | sh; then
          echo "acme.sh 安装失败，回退到自签名证书。"
        fi
      fi
      if [[ -x /root/.acme.sh/acme.sh ]] && /root/.acme.sh/acme.sh --set-default-ca --server letsencrypt >/dev/null && \
        /root/.acme.sh/acme.sh --issue -d "${SERVER_ADDRESS}" --standalone --httpport 80 \
        --server letsencrypt --certificate-profile shortlived --days 6 --force >/dev/null; then
        /root/.acme.sh/acme.sh --install-cert -d "${SERVER_ADDRESS}" \
          --key-file "${key_file}" --fullchain-file "${cert_file}" \
          --reloadcmd "systemctl restart sbox-panel" >/dev/null
        echo "Let’s Encrypt 证书申请成功。"
      else
        echo "Let’s Encrypt 证书申请失败，回退到自签名证书。"
      fi
    fi
  fi
