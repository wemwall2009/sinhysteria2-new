...
if /root/sbox/sing-box check -c /root/sbox/sbconfig_server.json; then
    ...
    install_shortcut
    show_client_configuration
    # Ask only when INSTALL_PANEL was not explicitly supplied. This keeps
    # interactive installs convenient without breaking unattended installs.
    install_panel_answer=${INSTALL_PANEL:-}
    if [[ -z ${install_panel_answer} && -t 0 ]]; then
        echo ""
    ...
    if [[ ${install_panel_answer} =~ ^([Yy]|true|TRUE|1)$ ]]; then
        panel_installer="$(dirname -- "${BASH_SOURCE[0]}")/panel/install-panel.sh"
        if [[ -f ${panel_installer} ]]; then
            bash "${panel_installer}" || error "Web 面板安装失败，请查看上面的错误信息。"
        else
            bash <(curl -fsSL https://github.com/wemwall2009/sinhysteria2-new/raw/main/panel/install-panel.sh) || \
                error "Web 面板安装失败，请检查网络后重试。"
        fi
    fi
